...
test("set width, height, margin, and type", function() {	
  ...
  equals(200, chart.options.chart.width, 'width');
  equals(100, chart.options.chart.height, 'height');
  same([10,20,30,40], chart.options.chart.margin, 'margin');
  equals(chart.options.chart.type, 'area', 'type');
});
...
test("when reading categories, skip the first THs; they're just headers of headers", function() {	
...
test("read series data from table", function() {	
	var data = [
  	...
  	$(data).each(function (i) {
  		same(Highcharts.chart.options.series[i].data, data[i], 'series ' + i); 
	});
});
...
test("read series data from table, differential mode", function() {	
	...
	equals(Highcharts.chart.options.plotOptions.area.stacking, 'normal', 'stacking should be enabled ("normal" mode)');
	equals(Highcharts.chart.options.yAxis.min, 23, 'Make sure the y axis starts at the right place');
});

test("should work with more than one TH per TR", function() {	
	$('#chart-target').allyChart({sourceTable: '#cd-ryr-table' });
