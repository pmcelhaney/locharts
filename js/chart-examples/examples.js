define(['../chart/chart', '../math/money'], function(chart, Money) {
	...
	$(function() {
		...
		$('#bar').chart({
			...
			layers: ["borders", ["y-axis markers", Money], "x-axis label separators", ["x-axis labels", ['1st term (12 mo)', '1 renewal (2 yr)', '2 renewals (3 yr)', '3 renewals (4 yr)']], "bars", "values above points", ["bubble", function(i, value) {
				return labels[i] + '<br>Earnings: ' + value.toString();
			}], "column hotspots"],
			marginBottom: 20,
			marginTop: 40,
			marginLeft: 100,
		...
		$('#line').chart({
			data: [Money(50598.54), Money(51204.25), Money(51817.21), Money(52437.51)],
			layers: ["borders", ["y-axis markers", Money], "x-axis label separators", ["x-axis labels", ['1st term (12 mo)', '1 renewal (2 yr)', '2 renewals (3 yr)', '3 renewals (4 yr)']], "lines", "dots"],
			marginBottom: 20,
			marginTop: 40,
			marginLeft: 100,
			marginRight: 10,
		...
		$('#area').chart({
			...
			layers: ["borders", ["y-axis markers", Money], "x-axis label separators", ["x-axis labels", ['1st term (12 mo)', '1 renewal (2 yr)', '2 renewals (3 yr)', '3 renewals (4 yr)']], "area", "dots"],
			marginBottom: 20,
			marginTop: 40,
			marginLeft: 100,
			marginRight: 50,
		...
		$('#differential-area').chart({
			...
			layers: ["borders", ["y-axis markers", Money], "x-axis label separators", ["x-axis labels", ['Deposit'].concat($('#raise-your-rate-data').find('tbody tr').find('th:eq(0)').map(function() {
				return $(this).text();
			}).toArray())], "differential area", "lines", "dots", ["bubble", function(i) {
			...
			xValues: $('#raise-your-rate-data').find('thead th:gt(1)').map(function() {
		...
		$('#pie-chart').chart({
			data: $('#wealth-by-type tbody tr td').map(function() {
				return Money(parseInt($(this).text(), 10));
			}).toArray(),
			layers: ["borders", "pie", ["pie labels", $('#wealth-by-type thead tr th').map(function() {
				return $(this).text();
			}).toArray()]],
			marginBottom: 20,
			marginTop: 20,
		...
		var randomTradingDays = function(n) {
				...
				var day;
				var lastClose = 30;
				var previousDate = new Date(2010, 1, 4);
				for (i = 0; i < n; i++) {
					day = TradingDay();
